...
func TestAsiaCountryCoordinates(t *testing.T) {
	tests := []struct {
		country string
		hasCoords bool
	}{
	...
	for _, tt := range tests {
		t.Run(tt.country, func(t *testing.T) {
			_, exists := asiaCountryCoordinates[tt.country]
...
func TestCoordinatesStructure(t *testing.T) {
	// Test that Tokyo coordinates are reasonable
	tokyo, ok := asiaCountryCoordinates["JP"]
	if !ok {
		t.Fatal("Tokyo coordinates not found")
	}

	// Tokyo should be around 35N, 139E
	if tokyo.Lat < 30 || tokyo.Lat > 40 {
		t.Errorf("Tokyo latitude %f seems incorrect (expected ~35)", tokyo.Lat)
	}
	if tokyo.Lon < 135 || tokyo.Lon > 145 {
		t.Errorf("Tokyo longitude %f seems incorrect (expected ~139)", tokyo.Lon)
	}
}
...
func TestFetchWeatherWithUnknownCountry(t *testing.T) {
	// Test that unknown country defaults to Tokyo
	_, ok := asiaCountryCoordinates["UNKNOWN"]
	if ok {
		t.Error("UNKNOWN country should not have coordinates")
	}

	// Verify JP (Tokyo) exists as the fallback
	_, ok = asiaCountryCoordinates["JP"]
	if !ok {
		t.Error("JP (Tokyo) should exist as fallback")
	}
}
...
func TestAllCountriesHaveValidCoordinates(t *testing.T) {
	for country, coords := range asiaCountryCoordinates {
